...
base_model = EfficientNetV2M(weights='imagenet', include_top=False, input_tensor=inputs)
...
model.load_weights("model_efficientnetv2m_penyakit_cabai.h5")
print("✅ Model EfficientNetV2M berhasil dimuat.")

labels = ["Bercak", "Gemini", "Kriting", "Sehat"]
pages  = ["bercak.html", "gemini.html", "kriting.html", "sehat.html"]

# ================= PREDIKSI =================
def predict_disease(img_path, threshold=0.7):
...
def predict_frame(frame, threshold=0.7):
...
def save_history():
    ...
    gambar = request.form['gambar']  # path gambar atau base64
